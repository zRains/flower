...
export interface ConfigureItem {
  id: string
  name: string
  type: 'TEXT' | 'OBJECT' | 'FLOAT' | 'INTEGER' | 'BOOLEAN' | 'ARRAY'
  required?: boolean
  ...
  return (
    <div className={style['api-service-panel-wrapper']}>
      ...
      <section className="api-service-inputs">
        <div className="api-service-inputs-label">所需参数：</div>
...
function ObjectConfigure(props: ApiServiceConfigureProps) {
  ...
  return (
    <section className={cn(style['object-configure'], { 'is-darker': isDarker }, 'object-configure-public')}>
      ...
      <main className="object-configure-item-container">
        {item.children?.map((child, idx) => (
          <ApiServiceConfigureItem key={idx} item={child} namePath={namePath} />
        ))}
      </main>
    </section>
  )
